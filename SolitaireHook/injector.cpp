...
BOOL dllInjector(const char* dllpath, DWORD pID)
{
    ...
    WriteProcessMemory(pHandle, (LPVOID)remoteString, dllpath, strlen(dllpath) + 1, NULL);
    HANDLE remoteThread =CreateRemoteThread(pHandle, NULL, NULL, (LPTHREAD_START_ROUTINE)remoteLoadLib, (LPVOID)remoteString, NULL, NULL);
    if (remoteThread == NULL) {
        VirtualFreeEx(pHandle, remoteString, 0, MEM_RELEASE);
        CloseHandle(pHandle);
        return false;
    }
